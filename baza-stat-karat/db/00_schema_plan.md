...
12. `process_operations` — справочник технологических операций производства.
13. `master_skills` — матрица навыков мастеров (связь мастеров и операций).
...
**Связи:**
- 1 ко многим с `order_execution` (через `order_execution.master_id`) — мастер может выполнять множество заказов.
- 1 ко многим с `qualification_history` (через `qualification_history.master_id`) — история изменений квалификации мастера.
- 1 ко многим с `master_skills` (через `master_skills.master_id`) — мастер может владеть множеством технологических операций.
...
### 3.12. Таблица `process_operations` (справочник технологических операций производства)

**Назначение:**
Справочник технологических операций производства для разных типов материалов. Используется как база для матрицы навыков мастеров (`master_skills`) и для анализа технологического процесса.

**Отличие от `operations_dictionary`:**
Таблица `operations_dictionary` (раздел 3.3) содержит операции с нормативным временем для калькулятора. Таблица `process_operations` — это более общий справочник технологических операций, который используется для описания навыков мастеров и анализа производственного процесса независимо от конкретных норм времени.

**Примеры записей:**
- Code: `acrylic_check_dimensions`, Name: "Проверка размеров деталей после ЧПУ", Material: acrylic
- Code: `acrylic_manual_polish`, Name: "Ручная полировка сложных участков", Material: acrylic
- Code: `acrylic_assembly_large`, Name: "Сборка крупногабаритных изделий (>3м)", Material: acrylic
...
|------|-----|---------|----------|
| `id` | uuid | PK | Идентификатор операции. |
| `code` | text | да, unique | Уникальный код операции (например, `acrylic_check_dimensions`). |
| `name` | text | да | Человекочитаемое название операции. |
| `description` | text | нет | Подробное описание операции, особенностей её выполнения. |
| `material_type` | text | да | Тип материала (например, `acrylic`, `quartz`). |
| `is_active` | boolean | да | Используется ли операция сейчас. `default true` |
| `created_at` | timestamptz | да | Когда операция добавлена в справочник. `default now()` |

**Особенности:**
- Поле `code` имеет уникальное ограничение (индекс/constraint) для предотвращения дублирования.
- Таблица используется для построения матрицы компетенций мастеров.
- В будущем может быть интегрирована с аналитикой тайм-трекера для оценки эффективности по типам операций.

**Связи:**
- 1 ко многим с `master_skills` (через `master_skills.process_operation_id`) — одна операция может быть освоена многими мастерами на разных уровнях.

---

### 3.13. Таблица `master_skills` (матрица навыков мастеров)

**Назначение:**
Матрица навыков мастеров — описывает, какие технологические операции может выполнять каждый мастер и на каком уровне. Используется для:
- анализа специализации мастеров,
- планирования назначения на редкие или сложные операции,
- будущей интеграции с аналитикой тайм-трекера (сравнение теории и факта по типам операций).

**Примеры записей:**
- Мастер "Иванов И.И." умеет выполнять операцию `acrylic_manual_polish` на уровне 5, это его основная операция
- Мастер "Петров П.П." умеет выполнять операцию `acrylic_check_dimensions` на уровне 3, не основная операция
- Мастер "Сидоров С.С." умеет выполнять операцию `acrylic_assembly_large` на уровне 4, основная операция, комментарий: "Ограничение до 4м по длине"

**Требования по полям:**

| Поле | Тип | Обязат. | Описание |
|------|-----|---------|----------|
| `id` | uuid | PK | Идентификатор записи навыка. |
| `master_id` | uuid | да | FK → `masters.id`. Мастер, владеющий навыком. |
| `process_operation_id` | uuid | да | FK → `process_operations.id`. Технологическая операция. |
| `skill_level` | int | да | Уровень владения операцией (например, от 1 до 5). |
| `is_primary` | boolean | да | Является ли операция основной специализацией мастера. `default false` |
| `comment` | text | нет | Дополнительные условия или ограничения (например, "только изделия до 2м", "требуется помощник"). |
| `created_at` | timestamptz | да | Когда навык добавлен в систему. `default now()` |

**Особенности и связи:**
- Связь многие-ко-многим между `masters` и `process_operations` через эту таблицу.
- Один мастер может иметь множество записей в `master_skills` (владеет несколькими операциями).
- Одна технологическая операция может быть освоена разными мастерами на разных уровнях.
- Рекомендуется создать составной индекс на `(master_id, process_operation_id)` для быстрого поиска навыков конкретного мастера.

**Связи:**
- Много к 1 с `masters` (через `master_id`) — у мастера может быть много навыков.
- Много к 1 с `process_operations` (через `process_operation_id`) — операция может быть у многих мастеров.
